...
def planning_eval(results, eval_config, logger):
    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        #instance_inds = data["instance_inds"]
        print(data.keys())
        ...
        planning_out = {
            'ego_trajs':sdc_planning[0,:, :6, :2].detach(),
            'gt_ego_fut_masks':sdc_planning_mask[0,:, :6, :2].detach(),
            'pred_trajs':pred_sdc_traj[:, :6, :2].detach(),
            'fut_boxes': fut_boxes #7
        }
        features_out = {
            'instance_feature':res['img_bbox']['instance_feature'].detach(),
            'map_instance_features':res['img_bbox']['map_instance_feature'].detach(),
            #'anchor_embed':res['img_bbox']['anchor_embed'].detach(),
        }
        planning_list.append(planning_out)
        features_list.append(features_out)
    print("saving features....")
    filename = 'features_eval_all_detach_with_boxes_with_our_boxes.pkl'
    with open(filename, 'ab') as f:
        pickle.dump(planning_list, f)
    filename2 = "planning_eval_all_detach_with_boxes_with_our_boxes.pkl"
